  ...
  <div class="app-container">
    <el-card shadow="never">
      ...
      <el-table ref="dataTableRef" v-loading="loading" :data="roleList" border
        ...
        <el-table-column label="图片" prop="images" min-width="150" align="center">
          <template #default="scope">
            <!-- 显示多个图片 -->
            <div v-if="scope.row.images" class="image-container">
              <template v-if="Array.isArray(scope.row.images)">
                <el-image 
                  v-for="(image, index) in scope.row.images" 
                  :key="index"
                  :src="`${baseUrl}${image}`" 
                  style="width: 50px; height: 50px; object-fit: contain; margin: 2px;"
                  :preview-src-list="scope.row.images.map(img => `${baseUrl}${img}`)" 
                  hide-on-click-modal 
                  preview-teleported 
                />
              </template>
              <template v-else>
                <el-image 
                  ...
                  :preview-src-list="[`${baseUrl}${scope.row.images}`]" 
                  hide-on-click-modal 
                  preview-teleported 
                />
              </template>
  ...
  if (data && data.id) {
    ...
    if (data.images) {
      if (Array.isArray(data.images)) {
        formData.initialImageUrls = data.images.map(img => `${baseUrl}${img}`)
        formData.originalImagePaths = [...data.images] // 保存原始相对路径
      } else {
        formData.initialImageUrls = [`${baseUrl}${data.images}`]
        formData.originalImagePaths = [data.images] // 保存原始相对路径
      }
    } else {
