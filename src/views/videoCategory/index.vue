<template>
  <div class="app-container">
    <el-card shadow="never">
      <div class="mb-10px">
        <div>
          <el-input v-model="queryParams.name" placeholder="请输入标题" style="width: 200px;margin-right: 10px;" />
      ...
      <el-table ref="dataTableRef" v-loading="loading" :data="roleList" border
        ...
        <el-table-column label="标题" prop="name" min-width="100" align="center" />
        ...
        <el-table-column label="视频" prop="videoUrl" min-width="100" align="center">
          <template #default="scope">
               <video 
                :src="`${baseUrl}${scope.row.videoUrl}`" 
    ...
    <el-dialog v-model="dialog.visible" :title="dialog.title" width="500px" @close="handleCloseDialog">
      <el-form ref="roleFormRef" :model="formData" :rules="rules" label-width="100px">
        ...
        <el-form-item label="封面图：">
          <imageUploads v-model="formData.image" :limit="1" :initial-url="formData.initialImageUrl"
            @file-changed="handleFileChanged" />
        </el-form-item>
        <el-form-item label="视频：">
          <VideoUpload v-model="formData.video" :limit="1" :initial-url="formData.existingVideoUrl"
            @file-changed="handleVideoChanged" />
        </el-form-item>
...
import { listBanner, addBanner, updateBanner, delBanner } from "@/api/videoCategory/index.js"
...
const rules = reactive({
  image: [{ required: false, message: "请上传封面图", trigger: "blur" }], 
...
function handleOpenDialog (data) {
  ...
  videoChanged.value = false // 重置图片变化标识
  ...
  if (data && data.id) {
    ...
    formData.video = null
    formData.sortOrder = data.sortOrder
    formData.initialImageUrl = data.imageUrl ? `${baseUrl}${data.imageUrl}` : '' 
    formData.existingVideoUrl = data.videoUrl ? `${baseUrl}${data.videoUrl}` : '' 

    console.log('编辑数据:', data)
    console.log('初始图片URL:', formData.initialImageUrl,formData.image)
    console.log('初始视频URL:', formData.existingVideoUrl,formData.video)
  } else {
    dialog.title = "新增"
    // 重置表单数据
    formData.name = ''
...
function handleVideoChanged (hasChanged) {
  videoChanged.value = hasChanged
  console.log('图片是否有变化:', videoChanged.value)
}
...
function handleSubmit () {
  ...
  roleFormRef.value.validate((valid) => {
    if (valid) {
      ...
      // if (dialog.title === "修改" && !formData.imageUrl && !formData.initialImageUrl) {
      //   ElMessage.error("请上传图片")
      //   return
      // }

      // // 新增时必须有图片
      // if (dialog.title === "新增" && !formData.imageUrl) {
      //   ElMessage.error("请上传图片")
      //   return
      // }
      ...
      if (dialog.title === "修改") {
        ...
        // 只有当图片有变化时才添加file参数
        if (imageChanged.value && formData.image) {
          formDatas.append('image', formData.image)
          console.log('添加新图片到FormData')
        }
        if (videoChanged.value && formData.video) {
